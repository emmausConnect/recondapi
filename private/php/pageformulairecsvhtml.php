...
<article style="border-style:inset;width:850px">
	<div class="div-saisie">
		...
		Pour tester l'algorithme, vous pouvez :
		<ul>
			<li>si vous avez beaucoup de cas, copier les colonnes Excel dans le champs ci dessous</li>
			<li>pour les cas ponctuels, renseigner les champs un par un</li>
		</ul>
	...
	<div class="div-saisie">
		<h3>Copie de colonnes Excel</h3>
		Copiez ci-dessous les colonnes de l'excel dans cet ordre :<br>
		CPU, taille disk 1, type disk 1 (HDD, SSD ...), taille disk 2, type disk 2 , taille RAM<br>
		séparés par une tabulation (se fait automatiquement avec le copier coller)<br>
		<br>
		<form action="exformtableau.php" method="post">
		<textarea id="text" name="text" rows="4" cols="100">
EOT;

// on remet la valeur saisie précédemment
	if (!$initial) {
		$retour .= $_POST["text"];
		$nl = "";
		if ($_POST["text"]!='') {
	...
	$retour .= '</textarea>';
	$retour .= '<h3>Calcul individuel</h3>';
